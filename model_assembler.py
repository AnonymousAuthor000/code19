    ...
    with fileinput.input(files=model_file, inplace=True) as f:
        for line in f:
                ...
                while(len(OpIDList)):
                    for i in range(len(OpIDList)):
                        if set(OpList[i]['input']) <= set(get_tensor):
                            ...
                            elif OpList[i]['OpName'] == 'ObfOptions':
                                print((' auto %s = %s::%s(%s, mapped_hash);' % (get_inout_string(OpList[i]['output']), OpList[i]['LayerID'], OpList[i]['LayerID'], get_inout_string(OpList[i]['input']))))
                            else:
                                print((' auto* %s = %s::%s(%s);' % (get_inout_string(OpList[i]['output']), OpList[i]['LayerID'], OpList[i]['LayerID'], get_inout_string(OpList[i]['input']))))
            ...
            if 'const Eigen::ThreadPoolDevice' in line:
                if enable_sig:
                    # The nb of threads should be 1 on android - have tested on real devices
            ...
            elif 'float* model' in line:
                if enable_sig:
                    print('float* model(float *input_v, float mapped_hash) {')
                else:
                    print('float* model(int *input_v, float mapped_hash) {')
            else:
                print(line, end="")
